...
contract MultiSig {
  ...
  function generateMessageToSignWith(address destination, uint256 value) public view returns (bytes32) {
    //the sequence should match generateMultiSigV2 in JS
    bytes32 message = keccak256(abi.encodePacked(address(this),address(0x0),destination, value, spendNonce));
